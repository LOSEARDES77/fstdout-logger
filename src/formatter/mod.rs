    ...
    pub fn format_stdout(&self, record: &Record) -> String {
        ...
        if self.config.show_file_info {
            ...
            if self.config.use_colors {
                ...
                format!("[{} {} {}] {}", timestamp.bright_black(), level_str, file_info, record.args())
            } else {
                format!("[{} {} {}:{}] {}", timestamp, level_str, file, line, record.args())
            }
        } else {
            // Simpler format without file info
            if self.config.use_colors {
                format!("[{} {}] {}", timestamp.bright_black(), level_str, record.args())
            } else {
